...
"Count Instruments" calls the Count method on the musical instrument dictionaries to generate the total counts of each instrument type.
Spectre.Console is configured to display the counts of each musical instrument type in a color-coded breakdown chart.
The total count of all instruments is generated by adding all values returned by the Count method.
A formula converts the instrument counts from an int to a double and generates the percentage that each instrument type occupies in the collection.
Math.Round is called to round the double back to the nearest integer.
The total count of all instruments in the collection and each instrument type's percentage of the collection are displayed by the console. 
