...
The total count of all instruments is generated by adding up all of the values returned by the Count method and is displayed by the console.
The instrument counts are then converted from an int to a double and a formula generates the percentage that each instrument type occupies in the collection.
Math.Round is called to round the double back to the nearest integer.
These percentages are displayed by the console. 
